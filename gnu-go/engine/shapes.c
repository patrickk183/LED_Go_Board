...
handle_joseki_patterns(struct pattern_attribute *attributes,
		       unsigned int sh_class, int move,
		       ...
		       int your_ndragons)
{
  ...
  if (sh_class & (CLASS_j | CLASS_t)) {
    ...
    if ((board_size >= 17) && (sh_class & CLASS_F)) {
		...
		void *data)
{
  ...
  unsigned int call_class = pattern->pat_class;

  /* Don't accept fuseki marked patterns while scoring. */
  if (doing_scoring && (call_class & CLASS_F))
    return;
   ...
   * in the pattern.
   */
  if ((call_class & (CLASS_B | CLASS_C | CLASS_c | CLASS_a | CLASS_d | CLASS_O
  ...
  gg_assert(!(call_class & CLASS_b));

  /* Pattern class e, expand to make territory. */
  if (call_class & CLASS_e) {
    add_expand_territory_move(move);
    TRACE("...expands territory\n");
  }
  ...
  if (call_class & CLASS_W) {
    TRACE("...worthwhile threat move\n");
    add_worthwhile_threat_move(move);
  }

  handle_joseki_patterns(pattern->attributes, call_class, move,
...
{
  int k, l;
  int call_class = pattern->pat_class;
