...
int reversi_main16(int player_count, int depth)
{
  ...
  depth = depth+2;
  char board [SIZE2][SIZE2] = {0};  /* The board           */
  int moves[SIZE2][SIZE2] = { 0 };    /* Valid moves         */
  int row = 0;                      /* Board row index     */
  int col = 0;                      /* Board column index  */
  int no_of_games = 0;              /* Number of games     */
  int no_of_moves = 0;              /* Count of moves      */
   ...
   do
   {
     ...
     do
     {
       ...
       { /*   It is the player's turn                    */
		 ...
		 /* It is the computer's turn                    */
         if(valid_moves2(board, moves, '@')) /* Check for valid moves */
         {
           invalid_moves = 0;               /* Reset invalid count   */
           printf("Othello is thinking!");
		   computer_move2(board, moves, '@', depth);
           no_of_moves++;                   /* Increment move count  */
         }
         else
         {
     ...
     /* Game is over */
     display2(board);  /* Show final board */
...
void display2(char board[SIZE2][SIZE2])
{
   ...
   RGBMatrix *matrix = new RGBMatrix(&io, SIZE2, 1, 1);
   Canvas *canvas = matrix;
   matrix->set_luminance_correct(true);
   ...
   for(row = 0; row < SIZE2; row++)
     for(col = 0; col < SIZE2; col++)
     {
       ...
       for(rowdelta = -1; rowdelta <= 1; rowdelta++)
         ...
         { 
           /* Don't check outside the array, or the current square */
           if(row + rowdelta < 0 || row + rowdelta >= SIZE2 ||
              col + coldelta < 0 || col + coldelta >= SIZE2 || 
                                       (rowdelta==0 && coldelta==0))
             continue;
 ...
 * Third parameter identifies the computer.                        *
 ************/
void computer_move2(char board[][SIZE2], int moves[][SIZE2], char player, int depth)
{
   int row = 0;                          /* Row index               */
   int col = 0;                          /* Column index            */
   int best_row = 0;                     /* Best row index          */
   ...
   int new_score = 0;                    /* Score for current move  */
   int score = 100;                      /* Minimum opponent score  */
   char temp_board[SIZE2][SIZE2];          /* Local copy of board     */
   int temp_moves[SIZE2][SIZE2];           /* Local valid moves array */
   char opponent = (player == 'O')? '@' : 'O'; /* Identify opponent */
   /* Go through all valid moves */
   for(row = 0; row < SIZE2; row++)
     for(col = 0; col < SIZE2; col++)
     {
       ...
       /* find valid moves for the opponent after this move */
       valid_moves2(temp_board, temp_moves, opponent);

       /* Now find the score for the opponents best move */
       new_score = best_move2(temp_board, temp_moves, opponent);
...
int best_move2(char board[][SIZE2], int moves[][SIZE2], char player)
{
   ...
   char new_board[SIZE2][SIZE2] = { 0 };  /* Local copy of board    */
   int score = 0;                       /* Best score             */
   int new_score = 0;                   /* Score for current move */
   ...
   for(row = 0 ; row<SIZE2 ; row++)
     for(col = 0 ; col<SIZE2 ; col++)
     {
       ...
       /* Get score for move */
       new_score = get_score2(new_board, player);  
